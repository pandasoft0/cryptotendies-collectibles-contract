...
import "./TendiesFactory.sol";
import "./ILootBox.sol";

/**
 * @title TendiesBox
 * TendiesBox - a randomized and openable lootbox of TendiesCards
 */
contract TendiesBox is ILootBox, Ownable, Pausable, ReentrancyGuard, TendiesFactory {
  ...
  constructor(
    ...
    setOptionSettings(Option.Basic, 3, [7300, 2100, 400, 100, 100], guarantees);
    // Note that tokens ids will be one higher than the indexes used here.
    guarantees[0] = 3;
    setOptionSettings(Option.Premium, 5, [7200, 2100, 400, 200, 100], guarantees);
    guarantees[2] = 2;
    guarantees[4] = 1;
    setOptionSettings(Option.Gold, 7, [7000, 2100, 400, 400, 100], guarantees);
  }
   ...
   * This is called via safeTransferFrom when TendiesBox extends TendiesFactory.
  ...
  function _mint(
    ...
    require(_canMint(msg.sender, _option, _amount), "TendiesBox#_mint: CANNOT_MINT");
  ...
  function _pickRandomAvailableTokenIdForClass(
    ...
    if (tokenIds.length == 0) {
      ...
      require(
        ...
        "TendiesBox#_pickRandomAvailableTokenIdForClass: NO_TOKEN_ON_PREMINTED_CLASS"
      );
      return 0;
    }
    ...
    if (classIsPreminted[classId]) {
      ...
      revert("TendiesBox#_pickRandomAvailableTokenIdForClass: NOT_ENOUGH_TOKENS_FOR_CLASS");
    } else {
